...
def svdEst(dataMat,user,simMeas,item,percentage):
    '''函数svdEst()的参数包含：数据矩阵、用户编号、物品编号和奇异值占比的阈值，数据矩阵的行对应用户，列对应物品，函数的作用是基于item的相似性对用户未评过分的物品进行预测评分'''
    n=shape(dataMat)[1]
	...
	# xformedItems表示物品(item)在k维空间转换后的值
    for j in range(n):
...
if __name__ == "__main__" :
    testdata=loadExData()
    result  = recommend(testdata,1,N=3,percentage=0.8)
    print ("编号为1的用户推荐评分较高的3件商品\n")
    print (result ) 

...
def plot_graph(G):
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=G.nodes(), node_size=300, node_color='r', font_size =50 , alpha=0.8)
    nx.draw_networkx_edges(G, pos, edgelist=edges,alpha =0.4)
    plt.title("dolphins relationship")
    plt.show()
...
#按照字母顺序排序
colorlist = sns.color_palette("hls",len(G.nodes))
...
def plot_node(output, title):
    for i in range(len(nodes)):
        node_name = nodes[i]
        plt.scatter(np.array(output)[i,0],np.array(output)[i,1] ,label=str(i),
                color = colorlist[i],alpha=0.5,s = 250)
        plt.text(np.array(output)[i,0],np.array(output)[i,1] ,i,

...
def fcp(predictions, verbose=True):
    ...
    #  predictions_u[user1 ]   = (user1 , est )  
    for u0, _, r0, est, _ in predictions:
        predictions_u[u0].append((r0, est))


    # 循环两次进行计算推荐结果的逆序程度
    # 通常进行topk推荐的时候只是关心topK的数据是否真的是用户真正感兴趣的item, 更加符合个个性化推荐的初衷
    # 对于(item_i ,item_j) , esti > estj and r0i > r0j 就是用户打分过的物品中， 原来评分高的item希望推荐的时候也高
    # 通过观察推荐结果的逆序程度来评价推荐结果的优劣
